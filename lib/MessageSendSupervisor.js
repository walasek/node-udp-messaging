...
class MessageSendSupervisor {
	...
	constructor(options){
		...
		this.last_sent_pos = 0;
		this.next_pos = 0;
		this.last_part_pos = -1;
		this.done = false;
		this.sendPart(0);
		this.resend_xval = setInterval(() => this.think(), RESEND_XVAL);
	...
	think(){
		...
		if(now - this.last_activity > RESEND_XVAL)
			this.sendPart(this.last_sent_pos);
	}
}
