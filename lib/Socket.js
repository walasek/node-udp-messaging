...
class MessagingSocket extends EventEmitter {
	...
	handleDatagram(data, rinfo){
		...
		debug(`Received ${data.length} bytes: %O`, message);
		if(message.data_part){
			...
			if(recv_handler){
				...
				if(recv_handler.done){
					...
					this.pending_recv.splice(this.pending_recv.indexOf(recv_handler), 1);
					delete this.pending_recv_map[message.data_part.message_id];
