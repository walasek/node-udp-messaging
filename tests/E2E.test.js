...
function makeSocketLossy(socket, loss_chance){
...
module.exports = async function(test){
	for(let lossy = 0; lossy <= 2; lossy++){
		...
		await test(`E2E ${lossy ? `${LOSS*100}% loss` : 'non lossy'} suite:`, async (t) => {
			await t.test('E2E #1 - can exchange small messages over stunned ports', async (t) => {
				...
				if(lossy){
					makeSocketLossy(sock1, LOSS);
					makeSocketLossy(sock2, LOSS);
				}
			...
			await t.test('E2E #3 - can exchange large messages over stunned ports', async (t) => {
				const sock1 = dgram.createSocket('udp4');
				const sock2 = dgram.createSocket('udp4');
				const h1 = new Socket({socket: sock1});
				const h2 = new Socket({socket: sock2});
				...
				try {
					...
					t.test(`E2E #3 Summary: Time elapsed ${elapsed/1000} sec. Bandwidth: ${(MSG.length+MSG2.length)/(elapsed)} Kb/s`, () => {});
				}finally{
					h1.close();
					h2.close();
				}
